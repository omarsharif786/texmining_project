...
def word_process(text, stem = True) :
...
tfidf_vectorizer = TfidfVectorizer( tokenizer= word_process,
                                    max_df=0.5,
                                    min_df=0.1, stop_words='english',
                                    use_idf=True)
vectorizer = CountVectorizer(tokenizer=word_process,
                            max_df = 0.5,
                            min_df = 0.1, 
                            stop_words = 'english')
tfidf_matrix = tfidf_vectorizer.fit_transform(data) 

#fit the vectorizer to corpus data
